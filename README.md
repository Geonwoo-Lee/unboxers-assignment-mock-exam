...
- 이 저장소는 직접 작업하지 말고 반드시 포크한 저장소에서 작업해 주세요.
- 제출용 포크 저장소는 `public`으로 열어 두거나, `private`으로 운영한다면 `hoyaaaa`에게 `read` 권한을 부여해 주세요.
- 프론트엔드와 백엔드를 별도 저장소로 나누지 말고, 하나의 저장소에서 함께 관리해 주세요.
- `pnpm-workspace.yaml` 구성을 꼭 확인하고, 워크스페이스 기준으로 설치와 빌드가 문제없이 동작하도록 구성해 주세요.
- 최종 제출 전에는 새로운 환경에서도 설치와 빌드가 실제로 되는지 반드시 확인해 주세요.
- 최종 제출 시간은 마지막 커밋 시간 기준으로 판단합니다.
- 작업을 모두 완료한 뒤 포크한 저장소에 최종 상태를 `push`해 주세요.
- 실행 방법은 반드시 `README.md`의 제일 마지막에 정리한 뒤 `push`해 주세요.
- 큰 문제가 없는 이상 서버 코드나 빌드 방식은 수정하지 말아 주세요.
- 서버 코드나 빌드 방식에 수정이 꼭 필요한 문제가 있다면 별도로 연락해 주세요.
